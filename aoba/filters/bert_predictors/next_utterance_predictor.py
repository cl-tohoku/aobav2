...
class NextUtterancePredictor:
    ...
    def __call__(self, contexts: List[str], candidates: List[str]) -> Optional[List[Tuple[float, str]]]:
        ...
        prediction = torch.softmax(output.logits, dim=1).cpu()[:, 1]
...
if __name__ == "__main__":
    """ bash
    $ python aoba/filters/bert_predictors.py --model /work01/slud_livechat_2020/mlm-checkpoint-43000-pytorch-model.bin
    """
    ...
    print(" ".join(contexts))
    print("\n".join(f"- {s:.4f}: {candidate}" for s, candidate in results))
